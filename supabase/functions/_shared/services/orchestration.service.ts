// supabase/functions/_shared/services/orchestration.service.ts
...
// SystemHealthMonitor import'u kaldırıldı - mock kullanılıyor
...
export async function processUserMessage(
    dependencies: {
        SystemHealthMonitor: {
            evaluateSystemHealth(): Promise<{ health_score: number }>;
        };
        ControlledHybridPipeline: typeof ControlledHybridPipeline;
    ...
    const initialVault = {};
    const transactionId = dependencies.generateId();

    // Logger oluştur
    const logger = new dependencies.LoggingService(transactionId, userId);
    ...
    const systemHealth = await dependencies.SystemHealthMonitor
    ...
    try {
        ...
        const result = await dependencies.ControlledHybridPipeline
            .executeComplexQuery(
                context,
                pipelineType,
            );
 ...
 */
export function determinePipelineType(
    eventType: string,
):
    | "deep_analysis"
    | "pattern_discovery"
 ...
 */
export function ensureHumanityReminder(
    result: string,
): string {
