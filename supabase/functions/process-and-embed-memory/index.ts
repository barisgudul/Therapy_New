...
export async function handleProcessAndEmbedMemory(
  req: Request,
...
### İSTENEN ÇIKTI FORMATI ###
{
  ...
  "stylometry_analysis": { "avg_sentence_length": "Ortalama cümle uzunluğu (kelime sayısı)", "lexical_density": "Kelime çeşitliliğinin 0.0-1.0 skoru", "pronoun_ratio": { "first_person_singular": "1. tekil şahıs zamirlerinin oranı (0.0-1.0)", "first_person_plural": "1. çoğul şahıs zamirlerinin oranı (0.0-1.0)" } }
        ...
        body: JSON.stringify({
          requests: [
            ...
            },
            {
              ...
              content: {
                parts: [{
                  text: `Yazım Stili: ${JSON.stringify(stylometry_analysis)}`,
                }],
              },
            },
          ],
    ...
    return new Response(
      JSON.stringify({
        message: "Zihinsel DNA başarıyla işlendi ve hafızaya kaydedildi.",
      }),
      {
...
if (import.meta.main) {
  Deno.serve((req) => handleProcessAndEmbedMemory(req));
}
