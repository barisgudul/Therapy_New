...
export const transcribeAudio = async (audioUri: string): Promise<string> => {
  console.log("🎤 [GCP-SERVICES] transcribeAudio başlatılıyor...", {
    audioUri,
  });
...
export const textToSpeech = async (text: string): Promise<string> => {
  console.log("🔊 [GCP-SERVICES] textToSpeech başlatılıyor...", {
    text: text.substring(0, 50) + "...",
  });

  try {
    // Teknik odaya sadece metni gönderiyoruz.
    const payload = { text, therapistId: "therapist1" };
    ...
    await FileSystem.writeAsStringAsync(tempUri, data.audioContent, {
      encoding: FileSystem.EncodingType.Base64,
    });
