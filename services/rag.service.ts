...
export async function addMemoryAsync(userId: string, content: string, metadata: Record<string, any> = {}): Promise<void> {
    ...
    if (error) {
        ...
        throw new Error(`Hafıza eklenemedi: ${error.message}`);
    }
}

// YENİ: Akıllı Re-ranking için yardımcı fonksiyon
async function rerankDocuments(question: string, documents: any[]): Promise<string> {
    ...
    console.log(`[RAG_RE-RANKING] ${documents.length} doküman arasından en alakalı 3 tanesi seçiliyor...`);
...
`;
    ...
    try {
        const selectedContext = await invokeGemini(rerankPrompt, AI_MODELS.FAST, {
            temperature: 0.3,
            maxOutputTokens: 1000
        });
        
        console.log(`[RAG_RE-RANKING] Re-ranking tamamlandı. Seçilen context uzunluğu: ${selectedContext.length}`);
        return selectedContext;
    } catch (error) {
        console.error("[RAG_RE-RANKING] Re-ranking hatası, ilk 3 doküman kullanılıyor:", error);
...
export async function debugRAGPipeline(userId: string, question: string): Promise<{
    step1_retrieval: any[];
    ...
    console.log(`[RAG_DEBUG] 📚 Adım 1: ${potentialDocs.length} doküman bulundu`);
...
export async function queryWithContext(userId: string, question: string, promptTemplate: PromptTemplate): Promise<string> {
    ...
    console.log(`[RAG_SERVICE] 📚 ${potentialDocs.length} potansiyel doküman bulundu.`);

    // ADIM 2: AKILLI ELEME (RE-RANKING) - En alakalı 3 dokümanı seç
    const bestContext = await rerankDocuments(question, potentialDocs);
    console.log(`[RAG_SERVICE] 🎯 Re-ranking tamamlandı. Final context uzunluğu: ${bestContext.length}`);
    ...
    try {
        ...
        throw new Error("RAG süreci sırasında bir hata oluştu. Lütfen tekrar deneyin.");
    }
}