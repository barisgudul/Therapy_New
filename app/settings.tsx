...
const SettingsCard = ({ icon, label, onPress }: { icon: keyof typeof Ionicons.glyphMap; label: string; onPress: () => void; }) => (
...
const FeaturedCard = ({ icon, label, subtitle, onPress }: { icon: keyof typeof Ionicons.glyphMap; label: string; subtitle: string; onPress: () => void; }) => (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.featuredCard, pressed && styles.cardPressed]}>
        <LinearGradient
...
export default function SettingsScreen() {
    ...
    return (
        <LinearGradient colors={['#FDFEFF', '#F4F6FF']} style={styles.container}>
            <ScrollView contentContainerStyle={styles.contentContainer}>
                ...
                <View style={styles.gridContainer}>
                    <SettingsCard icon="person-circle-outline" label="Profili Düzenle" onPress={() => router.push('/(settings)/profile')} />
                    <SettingsCard icon="shield-half-outline" label="Güvenlik" onPress={() => router.push('/(settings)/security')} />
                </View>
                ...
                <View style={styles.destructiveZone}>
                    <View style={styles.destructiveHeader}>
                        ...
                        <Text style={styles.destructiveTitle}>Tehlikeli Bölge</Text>
                    </View>
                    {isResetting ? (
                        ...
                        <Pressable onPress={handleResetData} style={({ pressed }) => [styles.destructiveButton, pressed && styles.cardPressed]}>
                            <Text style={styles.destructiveButtonText}>Tüm Verileri Sıfırla</Text>
                            <Ionicons name="trash-outline" size={20} color="#BE123C" />
                        </Pressable>
                    )}
                    <Pressable onPress={handleSignOut} style={({ pressed }) => [styles.destructiveButton, pressed && styles.cardPressed]}>
                        <Text style={styles.destructiveButtonText}>Güvenli Çıkış</Text>
...
const styles = StyleSheet.create({
    ...
    emailText: { fontSize: 16, color: '#64748B', marginBottom: 8, fontWeight: '500', },
    pageTitle: { fontSize: 32, fontWeight: 'bold', color: '#1E293B', },
    
    gridContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16, gap: 16},
    card: {
    ...
    cardLabel: { fontSize: 16, fontWeight: '600', color: '#1E293B', textAlign: 'center' },
    ...
    featuredIconContainer: { width: 52, height: 52, borderRadius: 26, backgroundColor: 'rgba(255,255,255,0.8)', justifyContent: 'center', alignItems: 'center', marginRight: 16, },
    featuredTextContainer: { flex: 1, },
    featuredLabel: { fontSize: 18, fontWeight: 'bold', color: '#4C1D95' },
    featuredSubtitle: { fontSize: 14, color: '#6D28D9', marginTop: 4, opacity: 0.8 },
    ...
    destructiveHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 },
    destructiveTitle: { fontSize: 18, fontWeight: 'bold', color: '#9F1239' },
    destructiveButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FFFFFF', padding: 16, borderRadius: 16, marginTop: 8, borderWidth: 1, borderColor: '#FEE2E2' },
    destructiveButtonText: { fontSize: 16, fontWeight: '600', color: '#BE123C' },
    
    footer: { alignItems: 'center', paddingTop: 40, borderTopWidth: 1, borderTopColor: 'rgba(226, 232, 240, 0.5)', marginTop: 40, },
