...
import { getEventsForLast, logEvent } from "../../services/event.service.ts";
    ...
    useEffect(() => {
        particles.forEach((p) => {
            p.y.value = withRepeat(
                withTiming(p.y.value - height * 1.2, { duration: p.duration }),
                -1,
                false,
            );
            p.x.value = withRepeat(
                withTiming(p.x.value + (Math.random() - 0.5) * 100, {
                    duration: p.duration,
                }),
                -1,
                true,
...
export default function MoodComparisonScreen() {
    ...
    const [theme, setTheme] = useState<
        { initial: ThemeType; final: ThemeType } | null
    >(null);
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const before = MOOD_LEVELS.find((m) =>
                    m.label === beforeMoodLabel
                ) || defaultMood;
                const after = MOOD_LEVELS.find((m) =>
                    m.label === afterMoodLabel
                ) || defaultMood;

                const initialTheme: ThemeType = {
                    bg: before.bg,
                    tint: before.color,
                    particleColor: before.particleColor,
                };
                ...
                if (after.id > before.id) {
                    finalTheme = {
                        bg: after.bg,
                        tint: after.color,
                        particleColor: after.particleColor,
                    };
                } else if (after.id < before.id) {
                    finalTheme = {
                        bg: after.bg,
                        tint: after.umutRengi,
                        particleColor: after.umutRengi,
                    };
                } else {
                    finalTheme = {
                        bg: after.bg,
                ...
                setData({
                    before,
                    after,
                    question: "Bu değişimi nasıl yorumlarsın?",
                    response:
                        "Bu derin düşünceleri paylaştığın için teşekkürler. Her yansıma, yolculuğumuzu daha da aydınlatıyor.",
                    answer: "",
                });
                setStep("reveal");
                ...
                Alert.alert(
                    "Hata",
                    "Karşılaştırma verileri yüklenirken bir sorun oluştu. Lütfen tekrar deneyin.",
                    [
    ...
    const handleStartReflection = () => {
        if (step === "reveal") {
            ...
            lotusProgress.value = withSpring(1, {
                damping: 18,
                stiffness: 120,
            });
            setStep("writing");
    ...
    const animatedFinalParticlesContainerStyle = useAnimatedStyle(() => ({
        opacity: transitionProgress.value,
    }));
    const animatedLotusTintStyle = useAnimatedStyle(() => ({
        tintColor: theme
            ? interpolateColor(transitionProgress.value, [0, 1], [
                theme.initial.tint,
                theme.final.tint,
            ])
            : defaultMood.color,
    ...
    const animatedLotusContainerStyle = useAnimatedStyle(() => {
        const size = interpolate(lotusProgress.value, [0, 1], [
            LOTUS_SIZE,
            TOP_LOTUS_SIZE,
        ]);
        const translateY = interpolate(lotusProgress.value, [0, 1], [
            0,
            -(height / 2) + (Platform.OS === "ios" ? 110 : 80),
        ]);
        ...
        <View style={styles.container}>
            ...
            <Animated.View
                style={[
                    ...
                    animatedFinalParticlesContainerStyle,
                ]}
            >
            ...
            <View style={styles.flexContainer}>
                <View style={styles.lotusPositioner}>
                    <Animated.View style={animatedLotusContainerStyle}>
                        <Animated.Image
                            ...
                            style={[styles.lotusImage, animatedLotusTintStyle]}
                        />
                    </Animated.View>
                </View>
                ...
                {step === "synthesis" && (
                    <Pressable
                        style={styles.fullScreenExitPressable}
                            ...
                            >
                                <Animated.View
                                    entering={FadeIn.duration(400).delay(100)
                                        .springify().damping(15).stiffness(100)}
                                ...
                                >
                                    ...
                                    <TextInput
                                        ref={textInputRef}
                                        style={[styles.input, {
                                            borderBottomColor: theme.final.tint,
                                            color: "#E2E8F0",
                                        }]}
                                    ...
                                    <TouchableOpacity
                                        style={[styles.submitButton, {
                                            backgroundColor: theme.final.tint,
                                        }]}
