...
export default function ChangePasswordScreen() {
    ...
    const isPasswordStrong = (password: string) => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/.test(password);

    // İSİM DÜZELTİLDİ: Fonksiyon artık neyi "handle" ettiğini söylüyor.
    const handleUpdatePassword = async () => {
        if (!currentPassword || !newPassword || !confirmNewPassword) {
            return Alert.alert('Eksik Alanlar', 'Lütfen tüm alanları doldurun.');
        }
        if (newPassword !== confirmNewPassword) {
            return Alert.alert('Şifreler Uyuşmuyor', 'Girdiğiniz yeni şifreler eşleşmiyor.');
        }
        if (!isPasswordStrong(newPassword)) {
            return Alert.alert('Zayıf Şifre', 'Şifre en az 8 karakter olmalı, büyük/küçük harf ve rakam içermelidir.');
        }
        if (currentPassword === newPassword) {
            return Alert.alert('Aynı Şifre', 'Yeni şifreniz, mevcut şifrenizle aynı olamaz.');
        }

        setIsLoading(true);
        try {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user?.email) throw new Error('Kullanıcı kimliği doğrulanamadı.');
            
            // HATA DEĞİŞKENLERİ DÜZELTİLDİ: Artık 'e1', 'e2' gibi saçmalıklar yok.
            const { error: signInError } = await supabase.auth.signInWithPassword({ email: user.email, password: currentPassword });
            if (signInError) throw new Error('Mevcut şifreniz yanlış. Lütfen kontrol ediniz.');
            
            const { error: updateError } = await supabase.auth.updateUser({ password: newPassword });
            if (updateError) throw updateError; // Supabase'den gelen hatayı direkt fırlat.
            
            Alert.alert('Başarılı', 'Şifreniz güvenle değiştirildi.', [{ text: 'Harika', onPress: () => router.back() }]);
        } catch (error: any) {
                ...
                <ScrollView contentContainerStyle={styles.contentContainer} keyboardShouldPersistTaps="handled">
                    ...
                    <View style={styles.headerContainer}>
                        <View style={styles.iconBackground}>
                            <Ionicons name="shield-checkmark-outline" size={32} color="#1E3A8A" />
                        </View>
                        <Text style={styles.title}>Şifre Güncelleme</Text>
                        <Text style={styles.subtitle}>Hesap güvenliğinizi en üst düzeyde tutun.</Text>
                    </View>
                    ...
                    <View style={styles.formContainer}>
                        ...
                        <Pressable 
                            style={({ pressed }) => [styles.submitButton, (isLoading || pressed) && styles.buttonDisabled]} 
                            onPress={handleUpdatePassword} 
                            disabled={isLoading}
                        >
                            {isLoading 
                                ? <ActivityIndicator color="#FFFFFF" /> 
                                : <Text style={styles.submitButtonText}>Şifreyi Güvenle Değiştir</Text>
                            }
                        </Pressable>
                    </View>
                </ScrollView>
...
const styles = StyleSheet.create({
    ...
    subtitle: { fontSize: 16, color: '#4B5563', marginTop: 8, textAlign: 'center', maxWidth: '90%' },
