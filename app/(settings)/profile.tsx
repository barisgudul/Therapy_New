...
const SelectorGroup: FC<{
    label: string;
    options: { value: RelationshipStatus; label: string }[];
    selectedValue: RelationshipStatus;
    onSelect: (value: RelationshipStatus) => void;
}> = memo(function SelectorGroup({ label, options, selectedValue, onSelect }) {
    return (
...
const FeaturedCard: FC = memo(function FeaturedCard() {
    ...
    const planMeta = useMemo(() => ({
        Premium: {
            ...
            color: "#5B21B6",
            gradient: ["#F5D0FE", "#E9D5FF"] as const,
        },
        "+Plus": {
            icon: "star" as const,
            color: "#075985",
    ...
    return (
        <Pressable
            ...
            style={(
                ...
                styles.button,
                (isDisabled || isSaving) && styles.buttonDisabled,
...
export default function ProfileScreen() {
    ...
    const handleInputChange = <K extends keyof LocalProfileState>(
        key: K,
        value: LocalProfileState[K],
    ) => {
    ...
    const handleSave = useCallback(() => {
        ...
        try {
            const currentProfile =
                (vault && typeof vault === "object" && "profile" in vault)
                    ? (vault.profile as Record<string, unknown>)
                    : {};
            const newVaultData = vault && typeof vault === "object"
